...
class AIQPG2024R1Importer {
    constructor() {
        this.dataDir = path.join(__dirname, 'data');
        this.counsellingDbPath = path.join(this.dataDir, 'counselling.db');
        this.excelPath = path.join(__dirname, '../counselling_data/AIQ_PG_2024/AIQ_PG_2024_R1.xlsx');
        this.importStats = {
            totalRows: 0,
            importedRecords: 0,
            skippedRecords: 0,
    ...
    async importAIQPG2024R1() {
        console.log('🚀 IMPORTING AIQ_PG_2024_R1.xlsx - INDIVIDUAL FILE IMPORT...');
        ...
        try {
            ...
            // Get counselling type ID for AIQ_PG
            const counsellingTypeId = await this.getCounsellingTypeId(db, 'AIQ_PG');
            
            // Get counselling round ID for Round 1
            const counsellingRoundId = await this.getCounsellingRoundId(db, 1);
            
            console.log(`🔗 Counselling Type ID: ${counsellingTypeId} (AIQ_PG)`);
    ...
    printImportSummary() {
        console.log('\n📊 IMPORT SUMMARY FOR AIQ_PG_2024_R1.xlsx:');
        console.log('=====================================');
...
if (require.main === module) {
    const importer = new AIQPG2024R1Importer();
    importer.importAIQPG2024R1()
        .then(() => {
            console.log('\n✅ AIQ_PG_2024_R1 import complete!');
        })
        .catch(error => {
            console.error('\n❌ AIQ_PG_2024_R1 import failed:', error.message);
        });
}

module.exports = { AIQPG2024R1Importer };
