...
class FinalCounsellingVerifier {
    constructor() {
    ...
    async verifyAllCounsellingData() {
        console.log('🔍 FINAL COMPREHENSIVE COUNSELLING DATA VERIFICATION...');
        console.log('=====================================');
    ...
    async getDatabaseCountForFile(file) {
        ...
        try {
            ...
            if (file.subdir.startsWith('AIQ_PG')) {
                ...
                params = [`${year}-${parseInt(year) + 1}`, `${year}-${(parseInt(year) + 1).toString().slice(-2)}`];
...
if (require.main === module) {
    const verifier = new FinalCounsellingVerifier();
    verifier.verifyAllCounsellingData()
        .then(() => {
            console.log('\n✅ Final counselling data verification complete!');
        })
        .catch(error => {
            console.error('\n❌ Final counselling data verification failed:', error.message);
        });
}

module.exports = { FinalCounsellingVerifier };
