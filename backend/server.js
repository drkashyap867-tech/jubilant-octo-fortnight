...
// Import the new Universal Cutoff API
const UniversalCutoffAPI = require('./universal-cutoff-api');
...
// Initialize Cutoff API
const cutoffAPI = new CutoffAPI();

// Initialize Universal Cutoff API
const universalCutoffAPI = new UniversalCutoffAPI();
...
app.get('/api/search/comprehensive', async (req, res) => {
    try {
        const { q: query, stream, courseType, branch, state, city, limit = 50 } = req.query;

        console.log('🚀 Comprehensive search request:', { query, stream, courseType, branch, state, city, limit });
...
// Get all available branches based on course selection
app.get('/api/dropdown/branches', async (req, res) => {
    try {
        const { course } = req.query;
        const branches = await searchEngine.getAvailableBranches(course);
        res.json({
            success: true,
            data: branches
        });
...
// ===== CUTOFF DATA API ENDPOINTS =====

// Universal Cutoff API endpoints
app.get('/api/cutoff/counselling-types', async (req, res) => {
    try {
        await universalCutoffAPI.connect();
        const types = await universalCutoffAPI.getAvailableCounsellingTypes();
        res.json(types);
    } catch (error) {
        console.error('Error getting counselling types:', error);
        res.status(500).json({ error: 'Failed to get counselling types' });
    } finally {
        await universalCutoffAPI.disconnect();
    }
});

app.get('/api/cutoff/years', async (req, res) => {
    try {
        await universalCutoffAPI.connect();
        const years = await universalCutoffAPI.getAvailableYears();
        res.json(years);
    } catch (error) {
...
app.get('/api/cutoff/rounds/:counsellingType/:year', async (req, res) => {
    try {
        const { counsellingType, year } = req.params;
        await universalCutoffAPI.connect();
        const rounds = await universalCutoffAPI.getAvailableRounds(counsellingType, parseInt(year));
        res.json(rounds);
    } catch (error) {
...
app.get('/api/cutoff/aiq-data', async (req, res) => {
    try {
        await universalCutoffAPI.connect();
        const data = await universalCutoffAPI.getAIQData();
        res.json(data);
    } catch (error) {
        console.error('Error getting AIQ data:', error);
        res.status(500).json({ error: 'Failed to get AIQ data' });
    } finally {
        await universalCutoffAPI.disconnect();
    }
});

app.get('/api/cutoff/state-data/:stateCode', async (req, res) => {
    try {
        const { stateCode } = req.params;
        await universalCutoffAPI.connect();
        const data = await universalCutoffAPI.getStateData(stateCode);
        res.json(data);
    } catch (error) {
        console.error('Error getting state data:', error);
        res.status(500).json({ error: 'Failed to get state data' });
    } finally {
        await universalCutoffAPI.disconnect();
    }
});

app.get('/api/cutoff/search', async (req, res) => {
    try {
        const filters = req.query;
        await universalCutoffAPI.connect();
        const results = await universalCutoffAPI.searchCutoffData(filters);
        res.json(results);
    } catch (error) {
        console.error('Error searching cutoff data:', error);
        res.status(500).json({ error: 'Failed to search cutoff data' });
    } finally {
        await universalCutoffAPI.disconnect();
    }
});

app.get('/api/cutoff/trends', async (req, res) => {
    try {
        const filters = req.query;
        await universalCutoffAPI.connect();
        const trends = await universalCutoffAPI.getCutoffTrends(filters);
        res.json(trends);
    } catch (error) {
        console.error('Error getting cutoff trends:', error);
        res.status(500).json({ error: 'Failed to get cutoff trends' });
    } finally {
        await universalCutoffAPI.disconnect();
    }
});

app.get('/api/cutoff/stats', async (req, res) => {
    try {
        const filters = req.query;
        await universalCutoffAPI.connect();
        const stats = await universalCutoffAPI.getStatistics(filters);
        res.json(stats);
    } catch (error) {
