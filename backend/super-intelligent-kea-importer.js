...
class SuperIntelligentKEAImporter {
  constructor() {
  ...
  async importKEAFile(filePath) {
    try {
      console.log(`\n🧠 SUPER INTELLIGENT KEA IMPORT: ${path.basename(filePath)}`);
      ...
      // Process KEA data with super intelligent algorithm
      await this.superIntelligentKEAParser(rawData, fileInfo);

      console.log(`✅ SUPER INTELLIGENT KEA processing completed for ${path.basename(filePath)}`);
  ...
  async superIntelligentKEAParser(rawData, fileInfo) {
    try {
      ...
      // Process each college column with super intelligence
      for (let colIndex = 0; colIndex < collegeHeaders.length; colIndex++) {
        ...
        console.log(`\n🔍 Processing College: ${collegeName}`);

        // Find or create college
        let collegeId = await this.findOrCreateCollege(collegeName);
        if (!collegeId) continue;

        // Process the data for this college with super intelligence
        await this.superIntelligentCollegeDataParser(rawData, colIndex, collegeId, fileInfo);
      }

    } catch (error) {
      console.error('Error in super intelligent KEA parser:', error.message);
    }
  }

  async superIntelligentCollegeDataParser(rawData, colIndex, collegeId, fileInfo) {
    try {
      ...
      for (let rowIndex = 1; rowIndex < rawData.length; rowIndex++) {
        ...
        if (this.isKEACourseRow(cellValue)) {
          ...
          if (rankBuffer.length > 0 && currentCourse && currentCategory && currentQuota) {
            await this.processRankBuffer(collegeId, currentCourse, currentCategory, currentQuota, rankBuffer, fileInfo);
            rankBuffer = [];
          }
      ...
      console.error('Error in super intelligent college data parser:', error.message);
    }
  }

  async processRankBuffer(collegeId, courseName, category, quota, rankBuffer, fileInfo) {
    try {
  ...
  isKEACategoryRow(value) {
    ...
    const categoryPatterns = [
      ...
      /^GENERAL$/i, /^OPEN$/i, /^UR$/i, /^EWS$/i, /^PH$/i, /^PWD$/i
    ];
  ...
  async importAllKEAData() {
    try {
      console.log('🚀 Starting SUPER INTELLIGENT KEA Data Import...');
      ...
      console.log('\n🎉 SUPER INTELLIGENT KEA import completed!');
      ...
      console.error('❌ SUPER INTELLIGENT KEA import failed:', error.message);
    } finally {
...
// Run the SUPER INTELLIGENT KEA import
const importer = new SuperIntelligentKEAImporter();
