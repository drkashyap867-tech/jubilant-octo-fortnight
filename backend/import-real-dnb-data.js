...
class RealDNBDataImporter {
    ...
    async importFromExcel(filePath) {
        try {
            console.log('🏥 Starting Real DNB Data Import...');
    ...
    validateRequiredColumns(headers) {
        const required = ['STATE', 'COLLEGE/INSTITUTE', 'COURSE', 'SEATS'];
    ...
    validateRecord(record) {
        ...
        if (!record['STATE'] || !record['COLLEGE/INSTITUTE'] || !record['COURSE']) {
            return false;
        }
    ...
    normalizeRecord(record) {
        ...
        normalized.collegeId = 0;
        normalized.collegeCode = '';
        normalized.city = this.extractCityFromCollegeName(record['COLLEGE/INSTITUTE']);
        normalized.hospitalType = 'Government'; // Default for DNB
        normalized.accreditation = 'NABH Accredited'; // Default for DNB
        normalized.courseType = 'Postgraduate'; // DNB is always postgraduate
        ...
        return new Promise((resolve, reject) => {
            ...
            db.get(checkQuery, [
                ...
                if (existing) {
                    ...
                    const updateQuery = `
                        UPDATE dnb_courses SET
                            state = ?, city = ?, hospital_type = ?, accreditation = ?,
                            total_seats = ?, general_seats = ?, obc_seats = ?, sc_seats = ?,
                            st_seats = ?, ews_seats = ?, updated_at = CURRENT_TIMESTAMP
                        WHERE id = ?
                    `;
                    
                    const self = this;
                    db.run(updateQuery, [
                        record.state, record.city, record.hospitalType, record.accreditation,
                        record.totalSeats, record.generalSeats, record.obcSeats, record.scSeats,
                        record.stSeats, record.ewsSeats, existing.id
                    ], function(err) {
                        if (err) reject(err);
                        else {
                    ...
                    db.run(insertQuery, [
                        ...
                        record.scSeats, record.stSeats, record.ewsSeats, record.quota,
    ...
    generateImportReport() {
        ...
        console.log('\n🎉 Real DNB data import completed successfully!');
    }
}

// Run if called directly
if (require.main === module) {
    const importer = new RealDNBDataImporter();
    
    // Check if file path is provided
    const filePath = process.argv[2];
    if (!filePath) {
        console.error('❌ Please provide the Excel file path:');
        console.error('   node import-real-dnb-data.js <excel-file-path>');
        process.exit(1);
    }
...
module.exports = { RealDNBDataImporter };
