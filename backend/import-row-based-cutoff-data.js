...
class RowBasedCutoffDataImporter {
    constructor() {
    ...
    async importExcelFile(filePath) {
        try {
            ...
            for (const record of jsonData) {
                await this.processRowRecord(record, counsellingType, year, round);
            }

            console.log(`✅ Processed ${jsonData.length} records from ${filename}`);
    ...
    async processRowRecord(record, counsellingType, year, round) {
        try {
            // Extract data from Excel record based on the row structure
            const collegeName = record['COLLEGE/INSTITUTE'] || record['College'] || record['Institution'] || 'Unknown';
            const courseName = record['COURSE'] || record['Course'] || record['Specialization'] || 'Unknown';
            const category = record['CATEGORY'] || record['Category'] || record['Reservation'] || 'UR';
            const quota = record['QUOTA'] || record['Quota'] || record['Seat Type'] || 'AIQ';
            const cutoffRank = parseInt(record['ALL INDIA RANK'] || record['Rank'] || record['Closing Rank'] || record['Cutoff Rank'] || 0);
            ...
            // Clean up college name (remove extra commas and addresses)
            const cleanCollegeName = collegeName.split(',')[0].trim();

            // Find or create college
            let collegeId = await this.findOrCreateCollege(cleanCollegeName);
            if (!collegeId) return;
    ...
    async importAllMissingData() {
        try {
            console.log('🚀 Starting Row-Based Cutoff Data Import...');
...
// Run the import
const importer = new RowBasedCutoffDataImporter();
