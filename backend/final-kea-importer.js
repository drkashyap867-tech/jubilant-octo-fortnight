...
class FinalKEAImporter {
  ...
  async importKEAFile(filePath) {
    try {
      console.log(`\n🎯 FINAL KEA IMPORT: ${path.basename(filePath)}`);
      ...
      // Process KEA data with FINAL algorithm
      await this.finalKEAParser(rawData, fileInfo);

      console.log(`✅ FINAL KEA processing completed for ${path.basename(filePath)}`);
  ...
  parseKEAFilename(filename) {
    ...
    // KEA_2023_MEDICAL_STRAY_CRAY_CLEANED.xlsx
  ...
  async finalKEAParser(rawData, fileInfo) {
    try {
      if (rawData.length < 3) return;

      // Process KEA data with FINAL algorithm
      await this.processFinalKEAData(rawData, fileInfo);

    } catch (error) {
      console.error('Error in final KEA parser:', error.message);
    }
  }

  async processFinalKEAData(rawData, fileInfo) {
    try {
      let currentCollege = null;
      let currentCourse = null;
      let currentCategory = null;
      let currentQuota = 'KEA'; // Default quota for KEA
      let rankBuffer = [];

      // Start from row 0 (first row contains college names)
      for (let rowIndex = 0; rowIndex < rawData.length; rowIndex++) {
        ...
        if (this.isKEACollegeRow(cellValue)) {
          ...
          if (rankBuffer.length > 0 && currentCourse && currentCategory) {
          ...
          currentQuota = 'KEA'; // Reset to default
      ...
      console.error('Error in final KEA data processor:', error.message);
    }
  }
  ...
  async importAllKEAData() {
    try {
      console.log('🚀 Starting FINAL KEA Data Import...');
      ...
      console.error('❌ FINAL KEA import failed:', error.message);
    } finally {
...
// Run the FINAL KEA import
const importer = new FinalKEAImporter();
